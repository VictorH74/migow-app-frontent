...
interface PostCtxProps {
    ...
    loadMoreComments: (
        excludeCommentId: CommentInterface['id'],
        endDate: string
    ) => () => Promise<void>;
...
export default function PostProvider(props: PostProviderProps) {
    ...
    const loadComments = React.useCallback(
        async (
            ...
            cb?: (comments: CommentInterface[]) => void
        ) => {
            serverFetch<ResponsePageInterface<CommentInterface>>(
                `/p-s/comments/${props.id}?${parsePaginationToParams(
                    pagination
                )}&excludeCommentId=${excludeCommentId}`,
                { next: { tags: [props.id + '-comments'] } }
            ).then((page) => {
                if (!page) throw new Error('page of post undefined');
                console.log(page);
    ...
    const loadMoreComments =
        (excludeCommentId: string, endDate: string) => () =>
            loadComments(
                excludeCommentId,
                {
                    pageNumber,
    ...
    const createComment = React.useCallback(
        async (content: string) => {
            const createdComment = await clientHTTP.createComment({
                content,
                postId: props.id,
            });
    ...
    const createDeleteReaction = async () => {
        if (!!props.currentUserReaction) {
            await serverFetch<void>(
                `/p-s/reactions/${props.currentUserReaction.id}`,
                {
                    method: 'DELETE',
                }
            );
    ...
    const createUpdateReaction = async (reactionType: ReactionTypeEnum) => {
        if (
            ...
            props.currentUserReaction.type === reactionType
        )
            return;

        const createdUpdatedReaction =
            await serverFetch<ReactionInterface.SimpleType>('/p-s/reactions', {
                method: 'POST',
                headers: {
        ...
        setPosts((prev) =>
            prev.map((p) =>
                p.id === props.id
                    ? {
                          ...p,
                          currentUserReaction: createdUpdatedReaction,
                          reactCount: p.reactCount + 1,
                      }
                    : p
            )
