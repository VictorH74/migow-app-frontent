...
const AvatarContainer = React.memo(function AvatarContainer() {
...
export default function NewPostFormModal() {
    const hook = useNewPostFormModal();

    if (hook.showAddPostModal || !!hook.sharedPost)
        return (
            <ModalContainer
                modalClassName="max-w-[900px] h-fit max-h-full overflow-auto"
                onClose={hook.closeAddPostModal}
            >
                <div className="p-4 flex flex-col items-center gap-3 h-full w-full">
                    ...
                    <form
                        className="w-full grow overflow-auto flex flex-col gap-3"
                        action=""
                        onSubmit={hook.handleSubmit}
                    >
                        <div className="flex justify-between items-center">
                            <React.Suspense
                                fallback={<Loading height={35} width={35} />}
                            >
                                <AvatarContainer />
                            </React.Suspense>
                            <SymetricHorizontalButtonList
                                btnData={hook.btnData}
                                renderDelay={200}
                        ...
                        <div
                            className={twMerge(
                                hook.files.length === 0 ? 'min-h-[300px]' : ''
                            )}
                        >
                            <textarea
                                autoFocus
                        ...
                        <div className="grid">
                            {hook.files.length === 0 ? (
                                <>
                                    <label
                                        ...
                                        className="text-gray-500 w-fit hover:cursor-pointer"
                                    >
                            ...
                            <div className="text-right">
                                <button
                                    type="submit"
                                    className="py-3 px-10 bg-gradient text-white font-semibold rounded-full"
                                >
                                    Create Post
                                </button>
                            </div>
