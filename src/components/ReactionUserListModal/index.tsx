...
export default function ReactionUserListModal(
    props: ReactionUserListModalProps
) {
    ...
    const tabArray = React.useMemo(() => {
        ...
        Object.entries(props.reactionCountByType).forEach(([k, v]) => {
            if (v > 0) array.push({ prop: k, value: v });
        });

        return array;
    }, [props.reactionCountByType]);

    const reactionByPropName: Record<
        keyof ReactionInterface.ReactionCountByType,
        { name: string; type: ReactionTypeEnum }
    > = React.useMemo(() => {
    ...
    return (
        <ModalContainer onClose={props.onClose}>
            <Tabs<UserInterface.ReactionUserType>
                generateChildren={(user) => (
                    <UserTile
                        key={user.id}
                        {...user}
                        reactionIcon={EMOJI_BY_REACTION_TYPE[user.reactionType]}
                    />
                )}
                tabGenerateArray={[
                    {
                        label: 'All',
                        queryFunc: (usernamePrefix) =>
                            clientHTTP.getAllReactionUser(
                                props.target,
                                usernamePrefix
                            ),
                    },
                    ...(tabArray.length > 1
                        ? tabArray.map((obj) => ({
                              label: reactionByPropName[
                                  obj.prop as keyof ReactionInterface.ReactionCountByType
                              ].name,
                              queryFunc: (usernamePrefix: string) =>
                                  clientHTTP.getAllReactionUser(
                                      props.target,
                                      usernamePrefix,
                                      reactionByPropName[
        ...
        <li className="flex justify-between px-1 py-2">
            <div className="flex items-center hover:cursor-pointer group">
                <div className="relative">
                    ...
                    <span className="absolute bg-white shadow-md rounded-full -bottom-1 right-0 text-2xls">
                        {props.reactionIcon}
                    </span>
                </div>
                <Link
