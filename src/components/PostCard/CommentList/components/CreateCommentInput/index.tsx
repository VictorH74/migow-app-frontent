...
function AvatarComponent(avatarSxProps?: SxProps) {
...
export default function CreateCommentInput(props: CreateCommentInputProps) {
    ...
    const handleSubmit = async () => {
        const formattedContent = commentInputValue.split("\n").join("<br>")
    ...
    return (
        <div className={twMerge("flex", props.containerClassname)}>
            <React.Suspense fallback={<MuiAvatar className="mr-2" sx={props.avatarSxProps} />}>
            ...
            <div className="w-full">
                ...
                <div className="flex items-center gap-1 text-sm">
                    {!!commentInputValue && (
                        <button className="bg-gradient text-white font-semibold px-4 py-1 rounded-lg" onClick={handleSubmit}>Submit</button>
                    )}
                </div>
            </div>
