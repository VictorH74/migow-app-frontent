...
const getCommonFriendship = async (
    currentUserId: string,
    profileOwnerUserId: string
) =>
    serverFetch<{
        count: number;
        firstsTwoFriends: [UserInterface.SimpleType, UserInterface.SimpleType];
    }>(
        `/u-s/friendships/common?userId=${currentUserId}&targetId=${profileOwnerUserId}`
    );

const checkIfHasFriendshipBetweenBoth = async (
    currentUserId: string,
    profileOwnerUserId: string
) =>
    serverFetch<{ friendshipStatus: FriendshipStatusEnum }>(
        `/u-s/friendships/${currentUserId}/friendship-with/${profileOwnerUserId}`
    );
...
export default async function ProfilePage({
    ...
    const ownerPrivacySettings = await serverFetch<PrivacySettingsInterface>(
        `/u-s/settings/${ownerUser.id}/privacy`
    );

    const friendshipBetweenBoth = await checkIfHasFriendshipBetweenBoth(
        currentUser.id,
        ownerUser.id
    );

    const hasFriendshipBetweenBoth =
        !isOwner &&
        friendshipBetweenBoth?.friendshipStatus ==
            FriendshipStatusEnum.IS_FRIEND;

    const commonFriendship = await getCommonFriendship(
        currentUser.id,
        ownerUser.id
    );
        ...
        <div className="mt-2">
            <main className=" w-full max-w-[1200px] m-auto">
                ...
                <div className=" flex gap-2">
                    {/* User IMG */}
                    <Avatar
                        image={ownerUser.profileImageUrl || ownerUser.name}
                        avatarSxProps={{
                            width: 176,
                            height: 176,
                            fontSize: 50,
                    ...
                    <div className="flex flex-col grow justify-center">
                        ...
                        {(ownerPrivacySettings?.bioVisibility ===
                            ...
                            <div className="w-full">
                                ...
                                adipisicing elit. Nostrum quo id aspernatur
                                voluptatum, eum adipisci debitis enim non illum
                                quasi possimus iusto libero eos consequuntur
                                commodi inventore sequi veritatis quaerat.
                            </div>
                        )}
                            ...
                            (commonFriendship.count > 2 ? (
                                <div className="flex items-center gap-6">
                                    <div className="relative">
                                        {commonFriendship.firstsTwoFriends.map(
                                            (f, i) => (
                                                <div
                                                    key={f.id}
                                                    className={
                                    ...
                                    <button className="hover:underline hover:underline-offset-2">
                                        and {commonFriendship.count - 2} other
                                        common friendships
                                    </button>
                                </div>
                            ) : (
                                <p>
                                    {commonFriendship.count} common friendships
                                </p>
                            ))}
                ...
                <UserActivities ownerUser={ownerUser} />
            </main>
        </div>
    );
}
